...
func parse (_ context: Context) -> Result<ContextValue> {
  ...
  case .Anchor:
    let m = peekMatch(context)
    let name = m.substring(from: m.characters.index(after: m.startIndex))
    let cv = parse(advance(context))
    let v = cv >>- getValue
...
func unwrapQuotedString (_ s: String) -> String {
  return s[s.characters.index(after: s.startIndex)..<s.characters.index(before: s.endIndex)]
}
...
func unescapeDoubleQuotes (_ input: String) -> String {
  return input
    ...
    |> replace(regex("\\\\x([0-9A-Fa-f]{2})"))
        { $ in String(describing: UnicodeScalar(parseInt($[1], radix: 16))) }
